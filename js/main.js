...
function getQuote(){
  ...
  fetch(url)
      ...
      .then(data => {
        console.log(data)
        const potentialPet = new PokeInfo (data.name,data.height,data.weight,data.types,
          data.sprites.other['official-artwork'].front_default,data.location_area_encounters)
          potentialPet.getTypes();
          potentialPet.isItHousePet();
          potentialPet.encounterInfo();
          let decision = ''
          if (potentialPet.housepet){
            decision = `This Pokemon is small enough,light enough, and safe enough to be a good pet! You can
            find ${potentialPet.name} in these area(S): `
            potentialPet.encounterInfo()
            document.getElementById('locations').innerText = '';
          } else {
            decision = `This Pokemon would not be a good pet because ${potentialPet.reason.join(' and ')} STAY AWAY! from
            these area's if untrained`
          }
...
class Poke {
  ...
  weightToPounds(weight){
    return Math.round(weight/4.536) / 100
  }
  ...
  isItHousePet() {
    // checks height,weight, and types
    let badTypes = ['fire','electric','poison','ghost']
